...
const categories = Array.from(new Set(aiComponents.map((c) => c.category))).sort()

export default function AIPage() {
  const [search, setSearch] = useState("")
  const [selectedCategory, setSelectedCategory] = useState<string | null>(null)

  const filtered = aiComponents.filter((c) => {
    const matchesSearch =
        ...
        <Stack gap="xl">
          <div>
            ...
            <p className="text-lg text-muted-foreground">
              34 specialized components for AI-powered applications. Agent cards, chat interfaces,
              execution tracking, task management, and AI workflow components.
            </p>
          </div>
          ...
          <div className="text-sm text-muted-foreground">
            Showing {filtered.length} of {aiComponents.length} components
          </div>

          <Grid cols={3} gap="md" className="grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
            {filtered.map((component) => (
              <a key={component.slug} href={`/ai/${component.slug}`} className="block group">
